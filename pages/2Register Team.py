...
def register():
    with st.form("form2", clear_on_submit=True):
        ...
        t_leader = text_field("Enter your name : ").lower()
        t_mail = text_field("Enter your Mail ID : ")
        t_phone = text_field("Enter your Phone No : ")
        password = text_field("Enter your Password : ", type = "password")
        submit2 = st.form_submit_button(label="Submit")  
    if submit2:
        name, loc= org.split(",")
        cursor.execute("INSERT INTO teamy (t_id, t_leader, t_mail, t_phone, t_password, org_name, org_loc) VALUES(?, ?, ?, ?, ?, ?, ?)", (t_id, t_leader, t_mail, t_phone, password, name, loc))
